...
class Descuentosmensuales extends CActiveRecord
{
	...
	{
		return 'TBL_CATDESCUENTOSMENSUALES';
	}
		...
		$sort->attributes = array(
			...
			'SUMATORIA'=>array(
				'asc'=>'(SELECT SUM("NOME_VALOR")
                                   FROM "TBL_CATPERSONASGENERALES" p, "TBL_CATCATEDRAS" c, "TBL_CATPERIODOSACADEMICOS" pa, "TBL_CATNOVEDADESMENSUALES" nm
								   WHERE t."DEME_ID" = nm."DEME_ID" AND nm."CATE_ID" = c."CATE_ID" AND c."PEAC_ID" = pa."PEAC_ID" AND pa."PEAC_ESTADO" = 1
								   AND c."PEGE_ID" = p."PEGE_ID" AND nm."NOME_VALOR">0 GROUP BY nm."DEME_ID"
					    )',
	...
	$connection = Yii::app()->db3;
     $sql = 'SELECT p."PEGE_IDENTIFICACION", p."PEGE_PRIMERNOMBRE", p."PEGE_SEGUNDONOMBRE", p."PEGE_PRIMERAPELLIDO", p."PEGE_SEGUNDOAPELLIDOS", dm."DEME_NOMBRE", nm."NOME_VALOR"			 
             FROM "TBL_CATPERSONASGENERALES" p, "TBL_CATCATEDRAS" c, "TBL_CATNOVEDADESMENSUALES" nm, "TBL_CATDESCUENTOSMENSUALES" dm, "TBL_CATPERIODOSACADEMICOS" pa 
             WHERE p."PEGE_ID" = c."PEGE_ID" AND dm."DEME_ID" = nm."DEME_ID" AND nm."CATE_ID" = c."CATE_ID" AND dm."DEME_ID" = '.$descuento.' AND nm."NOME_VALOR"!=0
	...
	public function resetDescuentomensual(){
	 $connection = Yii::app()->db3;
	 $sql = 'UPDATE "TBL_CATNOVEDADESMENSUALES" SET "NOME_VALOR" = 0 WHERE "DEME_ID" = '.$this->DEME_ID.' AND "NOME_VALOR" != 0;';
	...
	{
     $criteria = new CDbCriteria();
	 $criteria->select = 't."CATE_ID"';
	 $criteria->order = 't."CATE_ID"';
	 $Catedras = Catedras::model()->findAll($criteria);	 
	 ...
	 foreach($Catedras as  $Catedra){
		...
		$Novedadesmensuales->CATE_ID = $Catedra->CATE_ID;
