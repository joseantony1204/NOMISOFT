...
class Archivo
{
	...
	public function downloadFileUnidad($path,$file,$unidades)
    {
	 ...
	 "Echo.".$archivo->enter().
     "Echo CODIGO  FACULTAD ".$archivo->enter()."".$archivo->enter()."".$archivo->enter().
	 "Echo.".$archivo->enter();
	 $zipfile= new zipfile();
	 ...
	 foreach($unidades as $unidad){		
		$txt.="
		Echo ".$archivo->centro($unidad["FACU_ID"],8)."".$unidad["FACU_NOMBRE"]."".$archivo->enter();
		$opcion.="
		if %menup%==".$unidad["FACU_ID"]." print ".$realPath.$unidad["FACU_ID"].".dat".$archivo->enter();
		$zipfile->add_file(implode("",file($path.$unidad["FACU_ID"].".dat")),$path.$unidad["FACU_ID"].".dat");
	    
		$opcion1010.= $path.$unidad["FACU_ID"].".dat ";
