...
$objPHPExcel = $objReader->load("CATEDRATICOS.xlsx");
...
echo "<h2>IMPORTAR DATOS</h2>";
 ...
 if($Personasgenerales = Personasgenerales::model()->findByPk($Personasgeneral->PEGE_ID)){ 
  ...
  $Personasgenerales->PEGE_PRIMERAPELLIDO =  trim($data[$k][1]);
  $Personasgenerales->PEGE_SEGUNDOAPELLIDOS =  trim($data[$k][2]);
  $Personasgenerales->PEGE_PRIMERNOMBRE =  trim($data[$k][3]);
  $Personasgenerales->PEGE_SEGUNDONOMBRE =  trim($data[$k][4]);
  $Personasgenerales->PEGE_EMAIL =  trim($Personasgenerales->PEGE_EMAIL);
  
   if($Personasgenerales->save()){  
    echo $x.' - '.$Personasgenerales->PEGE_ID.' '.$Personasgenerales->PEGE_IDENTIFICACION.' '.$Personasgenerales->PEGE_PRIMERAPELLIDO.' '.$Personasgenerales->PEGE_SEGUNDOAPELLIDOS.' '.$Personasgenerales->PEGE_PRIMERNOMBRE.' '.$Personasgenerales->PEGE_SEGUNDONOMBRE.' '.$Personasgenerales->PEGE_EMAIL.'<br>';   
   }else{
