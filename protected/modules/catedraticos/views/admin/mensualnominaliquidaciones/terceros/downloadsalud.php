...
$columnas = count($Mensualnominaliquidaciones->prestaciones[1]);
 ...
 $encabezado=$arc->izquierda("UNIVERSIDAD DE LA GUAJIRA",90).$arc->enter().
	         ...
	         $arc->izquierda("RELACION DE DESCUENTO DE SALUD",90).$arc->izquierda("FECHA PROCESO: ".date("d/m/Y"),60).$arc->enter().
	         $arc->izquierda($tercero,90)."PAGINA       : ";

 ////Titulo
 $titulo = $arc->enter().$arc->enter().
           $arc->centro("CEDULA",$c1).$arc->centro("NOMBRES",$c2).$arc->centro("EPS",$c3).$arc->centro("I.B.C",$c4).$arc->centro("SALUD",$c5).$arc->centro("PATRONO",$c6).$arc->centro("T. APORTE",$c7).
		   $arc->enter().$arc->enter();
 $pagina=1;
