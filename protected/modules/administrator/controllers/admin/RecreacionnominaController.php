...
class RecreacionnominaController extends Controller
{
	...
	public function actionSetEstado($id)
	{
		...
		$Recreacionnomina=$this->loadModel($id);		
		if($Recreacionnomina->RENO_ESTADO==1){
		 Yii::app()->user->setFlash('warning','<strong>Oppss!. </strong> La Nomina de: <strong>'.$Recreacionnomina->RENO_PERIODO.'</strong> se encuentra <strong>PAGADA</strong>, por lo tanto no se puede modificar el estado.');
		}else{
		      $Recreacionnomina->RENO_ESTADO = 1;
			  $Recreacionnomina->RENO_FECHACAMBIO =  date('Y-m-d H:i:s');
			  $Recreacionnomina->RENO_REGISTRADOPOR = Yii::app()->user->id;
		      $Recreacionnomina->save();
			  Yii::app()->user->setFlash('success',' EL estado de la nomina de: <strong>'.$Recreacionnomina->RENO_PERIODO.'</strong> ha sido cambiado a <strong>PAGADA</strong> exitosamente...');
			 }
