...
$columnas = count($Semestralnominaliquidaciones->prestaciones[1]);
 ...
 for($i=1;$i<$filas;$i++){		 
  ...
  $lista[$index]=$arc->centro(number_format($Semestralnominaliquidaciones->prestaciones[$i][2]),$c1).$arc->centro($nombre,$c2).$arc->centro(trim($Semestralnominaliquidaciones->prestaciones[$i][7]),$c3).$arc->derecha(number_format($Semestralnominaliquidaciones->prestaciones[$i][8]),$c4);
  
  $total4=$total4+$Semestralnominaliquidaciones->prestaciones[$i][8]; // total
  $index++;  
 }
