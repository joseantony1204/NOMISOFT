<table width="100%" border="0" align="center">
     <tr>
	  ...
	  <?php $form=$this->beginWidget('bootstrap.widgets.TbActiveForm',array(
	    ...
	    <?php $data = array('04'=>'PRIMA DE NAVIDAD'); ?>
	...
	'id'=>'aumentodiaspn-grid',
	'dataProvider'=>$Empleosplanta->unidadespnavidadDataProvider,
	...
	'columns'=>array(
		 ...
		 array(
             'name'=>'MESES',
		     'filter'=>false,
		     'value'=>'CHtml::textField("numDiaspNavidad[$data[NOPN_ID]]",$data[NOPN_MESES],array("class"=>"span1","style"=>"text-align: center",))',
             'type'=>'raw',
...
<?php
Yii::app()->clientScript->registerScript('sendpn','
var btnModal = $("#confirmarform");
	

var updateGridpn = function()
{
	var url = "diaspnavidad";
	$.ajax({	
			url:url,
			type:"POST",
			data: $("#aumentodiaspn-form").serialize(),
			success:function(data){
								   $("#aumentodiaspn-form")[0].reset();
								   $("#aumentodiaspn-grid").yiiGridView("update",{});
								   //alert("Dias en prima de navidad actualizados correctamente...");						
							      },	
			});
}	
	
// CARGAR MODAL
$("#primanavidad").on("click",function(){
		if(confirm(" Esta seguro de actualizar los dias a la prima de navidad a todos los empleados que se deszpliegan en la lista? "))
        {
		 $("#pass-form")[0].reset();
		 $("#myModal").modal("show");										  		
        }
