<table width="100%" border="0" align="center">
	...
	'id'=>'aumentodiasv-grid',
	'dataProvider'=>$Empleosplanta->unidadesvacacionesDataProvider,
	...
	'columns'=>array(
		 ...
		 array(
             'name'=>'DIAS',
		     'filter'=>false,
		     'value'=>'CHtml::textField("numDiasVacaciones[$data[NOVA_ID]]",$data[NOVA_DIAS],array("class"=>"span1","style"=>"text-align: center",))',
             'type'=>'raw',
...
<?php
Yii::app()->clientScript->registerScript('sendv','
var btnModal = $("#confirmarform");
	

var updateGridv = function()
{
	var url = "diasvacaciones";
	$.ajax({	
			url:url,
			type:"POST",
			data: $("#aumentodiasv-form").serialize(),
			success:function(data){
								   $("#aumentodiasv-form")[0].reset();
								   $("#aumentodiasv-grid").yiiGridView("update",{});
								  // alert("Dias en vacaciones actualizados correctamente...");						
							      },	
			});
}	
	
// CARGAR MODAL
$("#vacaciones").on("click",function(){
		if(confirm(" Esta seguro de actualizar los dias a las vacaciones a todos los empleados que se deszpliegan en la lista? "))
        {
		 $("#pass-form")[0].reset();
		 $("#myModal").modal("show");										  		
        }
