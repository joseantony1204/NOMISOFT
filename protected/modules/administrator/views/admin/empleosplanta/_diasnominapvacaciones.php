<table width="100%" border="0" align="center">
     <tr>
	  ...
	  <?php $form=$this->beginWidget('bootstrap.widgets.TbActiveForm',array(
	    ...
	    <?php $data = array('03'=>'PRIMA VACACIONES'); ?>
		...
		<?php 
         ...
         $criteria->condition = '"TICA_ID" = 2';
		 $data = CHtml::listData(Tiposcargos::model()->findAll($criteria),'TICA_ID','TICA_NOMBRE'); 
		?>
        <?php echo $form->labelEx($Cform,'NOVE_TIPOCARGO'); ?>	 
	    <select name='Cform_NOVE_TIPOCARGO' id='Cform_NOVE_TIPOCARGO' onchange='this.form.submit();' class="span4">
	            <?php
			    foreach($data as $dat => $da)
	            {
				 echo "<option value='".$dat."'"; 
	...
	'id'=>'aumentodiaspv-grid',
	'dataProvider'=>$Empleosplanta->unidadespvacacionesDataProvider,
	...
	'columns'=>array(
		 ...
		 array(
             'name'=>'DIAS',
		     'filter'=>false,
		     'value'=>'CHtml::textField("numDiaspVacaciones[$data[NOPV_ID]]",$data[NOPV_DIAS],array("class"=>"span1","style"=>"text-align: center",))',
             'type'=>'raw',
...
<?php
Yii::app()->clientScript->registerScript('sendpv','
var btnModal = $("#confirmarform");
	

var updateGridpv = function()
{
	var url = "diaspvacaciones";
	$.ajax({	
			url:url,
			type:"POST",
			data: $("#aumentodiaspv-form").serialize(),
			success:function(data){
								   $("#aumentodiaspv-form")[0].reset();
								   $("#aumentodiaspv-grid").yiiGridView("update",{});
								   //alert("Dias en prima de vacaciones actualizados correctamente...");						
							      },	
			});
}	
	
// CARGAR MODAL
$("#primavacaciones").on("click",function(){
		if(confirm(" Esta seguro de actualizar los dias a la prima de vacaciones a todos los docentes que se deszpliegan en la lista? "))
        {
		 $("#pass-form")[0].reset();
		 $("#myModal").modal("show");										  		
        }
