<?php
$filas = count($Vacacionesnominaliquidaciones->prestaciones);
$columnas = count($Vacacionesnominaliquidaciones->prestaciones[1]);
 ...
 $lista = NULL; $index=0;
 for($i=1;$i<$filas;$i++){		 
  $nombre = trim($Vacacionesnominaliquidaciones->prestaciones[$i][3]).$arc->espacio(1).trim($Vacacionesnominaliquidaciones->prestaciones[$i][4]).$arc->espacio(1).trim($Vacacionesnominaliquidaciones->prestaciones[$i][5]).$arc->espacio(1).trim($Vacacionesnominaliquidaciones->prestaciones[$i][6]);				 
  $lista[$index]=$arc->centro(number_format($Vacacionesnominaliquidaciones->prestaciones[$i][2]),$c1).$arc->centro($nombre,$c2).$arc->centro(trim($Vacacionesnominaliquidaciones->prestaciones[$i][7]),$c3).$arc->derecha(number_format($Vacacionesnominaliquidaciones->prestaciones[$i][8]),$c4);
  
  $total4=$total4+$Vacacionesnominaliquidaciones->prestaciones[$i][8]; // total
  $index++;  
 }
