...
$Periodo = $model->PVNO_PERIODO;
$sql = ' pvn."PVNO_ID" = '.$model->PVNO_ID;
$unidades = $Primavacacionesnominaliquidaciones->getUnidadesEnNomina($sql);

foreach($unidades as $unidad){
 $Unidades = Unidades::model()->findByPk($unidad["UNID_ID"]);
 $query = ' '.$sql.' AND u."UNID_ID" =  '.$Unidades->UNID_ID;
 $Primavacacionesnominaliquidaciones->mostrarLiquidacion($query);
 ...
 $encabezado = $arc->izquierda("UNIVERSIDAD DE LA GUAJIRA",85).$arc->enter().
			   ...
			   $arc->izquierda("INFORME DE PRIMA DE VACACIONES",85).$arc->izquierda("FECHA PROCESO: ".date("d/m/Y"),60).$arc->enter().
 ...
 for($i=1;$i<$filas-1;$i++){ 
  $neto = ($Primavacacionesnominaliquidaciones->liquidacion[$i][$columnas-1])-(($Primavacacionesnominaliquidaciones->parafiscales[$i][$parafisales-1])+($tblD[$i][$columnasTblD-1]));
  if($neto!=0){
	...
	$basico=NULL;
	$basico[0]=$arc->espacio($c1).chr(179).$arc->izquierda("Comprobante No. ".$Primavacacionesnominaliquidaciones->liquidacion[$i][13].'-'.$Primavacacionesnominaliquidaciones->liquidacion[$i][1],$c2);
	$basico[1]=$arc->espacio($c1).chr(179).$arc->izquierda(trim($Primavacacionesnomina->PVNO_PERIODO), $c2);
	...
	$pago=NULL;
	$pago[0]=$arc->izquierda("SUELDO (".number_format($Primavacacionesnominaliquidaciones->liquidacion[$i][4]).")",$c3).chr(179).$arc->centro($Primavacacionesnominaliquidaciones->liquidacion[$i][2],$c4).chr(179).$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$i][5]),$c5).chr(179).$arc->espacio($c6).chr(179).$arc->espacio($c7);
	$lineapago=1;
		...
		for ($ln=6;$ln<13;$ln++){
		  if ($Primavacacionesnominaliquidaciones->liquidacion[$i][$ln]!=0){
		   ...
		   $pago[$lineapago]=$arc->izquierda($Primavacacionesnominaliquidaciones->liquidacion[0][$ln],$c3).chr(179).$arc->centro($antig,$c4).chr(179).$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$i][$ln]),$c5).chr(179).$arc->espacio($c6).chr(179).$arc->espacio($c7);
		   $lineapago++;
		  }
		}
		...
		for ($ln=1;$ln<2;$ln++){
			if ($Primavacacionesnominaliquidaciones->parafiscales[$i][$ln]!=0){
				$pago[$lineapago]=$arc->izquierda($Primavacacionesnominaliquidaciones->parafiscales[0][$ln],$c3).chr(179).$arc->espacio($c4).chr(179).$arc->espacio($c5).chr(179).$arc->derecha(number_format($Primavacacionesnominaliquidaciones->parafiscales[$i][$ln]),$c6).chr(179).$arc->espacio($c7);
				$lineapago++;
			}
		}
		...
		$pago[$lineapago].=$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$i][$columnas-1]),$c5).chr(179);
		$pago[$lineapago].=$arc->derecha(number_format(($Primavacacionesnominaliquidaciones->parafiscales[$i][$parafisales-1])+($tblD[$i][$columnasTblD-1])),$c6).chr(179);
		$pago[$lineapago].=$arc->derecha(number_format(($Primavacacionesnominaliquidaciones->liquidacion[$i][$columnas-1])-(($Primavacacionesnominaliquidaciones->parafiscales[$i][$parafisales-1])+($tblD[$i][$columnasTblD-1]))),$c7);
		...
		$arc->escribir(chr(192).$arc->lineaH($c1).chr(197).$arc->lineaH($c2).chr(197).$arc->lineaH($c3).chr(197).$arc->lineaH($c4).chr(197).$arc->lineaH($c5).chr(197).$arc->lineaH($c6).chr(197).$arc->lineaH($c7).chr(217));
		$arc->enterH();		
  } 
 }
 $linpag=67-$linpag-1;
 ...
 $encabezado=$arc->izquierda("UNIVERSIDAD DE LA GUAJIRA",90).$arc->enter().
	         ...
	         $arc->izquierda("INFORME DE PRIMA DE VACACIONES",90).$arc->izquierda("FECHA PROCESO: ".date("d/m/Y"),60).$arc->enter().
 ...
 $detalle1=NULL;
 $detalle1[0]=$arc->izquierda("SUELDO BASICO------------------->".$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$filas-1][5]),16)."",80);
 $detalle1[1]=$arc->izquierda("GASTOS DE REPRESENTACION-------->".$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$filas-1][10]),16)."",80);
 $detalle1[2]=$arc->izquierda("SUBSIDIO DE TRANSPORTE---------->".$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$filas-1][7]),16)."",80);
 $detalle1[3]=$arc->izquierda("PRIMA DE ANTIGUEDAD------------->".$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$filas-1][6]),16)."",80);
 $detalle1[4]=$arc->izquierda("PRIMA TECNICA------------------->".$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$filas-1][9]),16)."",80);
 $detalle1[5]=$arc->izquierda("PRIMA DE ALIMENTACION----------->".$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$filas-1][8]),16)."",80);
 $detalle1[6]=$arc->izquierda("BONIFICACION SERVIC. PRESTADOS-->".$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$filas-1][11]),16)."",80);
 $detalle1[7]=$arc->izquierda("RIMA SEMESTRAL------------------>".$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$filas-1][12]),16)."",80);
 $detalle1[8]=$arc->izquierda("TOTAL DEVENGADO:---------------->".$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$filas-1][15]),16)."",80);
 ...
 $detalle2=NULL;  $suma = 0;
  $detalle2[0]=$arc->izquierda("RETENCION EN LA FUENTE--------->".$arc->derecha(number_format($Primavacacionesnominaliquidaciones->parafiscales[$filas-1][1]),15)."",50);
  ...
  $totalapr=round($var); $var=$Primavacacionesnominaliquidaciones->getInteresesCesantias($Primavacacionesnominaliquidaciones->liquidacion[$filas-1][$columnas-1]);
  $detalle2[$lindet2]=$arc->izquierda("INTERESES CESANTIAS (1%)",31,"-").">".$arc->derecha(number_format($var),15); $lindet2++;
  $totalapr+=round($var);$var=$Primavacacionesnominaliquidaciones->getPrimaNavidad($Primavacacionesnominaliquidaciones->liquidacion[$filas-1][$columnas-1]);
  $detalle2[$lindet2]=$arc->izquierda("PRIMA DE NAVIDAD (8.33%)",31,"-").">".$arc->derecha(number_format($var),15); $lindet2++;
  $totalapr+=round($var);$var=$Primavacacionesnominaliquidaciones->getPrimaVacaciones($Primavacacionesnominaliquidaciones->liquidacion[$filas-1][$columnas-1]);
  $detalle2[$lindet2]=$arc->izquierda("PRIMA DE VACACIONES (4.17%)",31,"-").">".$arc->derecha(number_format($var),15); $lindet2++;
  $totalapr+=round($var);$var=$Primavacacionesnominaliquidaciones->getVacaciones($Primavacacionesnominaliquidaciones->liquidacion[$filas-1][$columnas-1]);
  $detalle2[$lindet2]=$arc->izquierda("VACACIONES (4.17%)",31,"-").">".$arc->derecha(number_format($var),15); $lindet2++;
  $varPS=$Primavacacionesnominaliquidaciones->getPrimaSemestral($Primavacacionesnominaliquidaciones->liquidacion[$filas-1][$columnas-1]);
  ...
  $totalapr+=round($var);$var=$Primavacacionesnominaliquidaciones->getCajaCompensacion($baseicbf);
