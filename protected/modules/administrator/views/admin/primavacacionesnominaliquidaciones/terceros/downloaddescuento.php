<?php
$filas = count($Primavacacionesnominaliquidaciones->prestaciones);
$columnas = count($Primavacacionesnominaliquidaciones->prestaciones[1]);
 ...
 $titulo = $arc->enter().$arc->enter().
           $arc->centro("CEDULA",$c1).$arc->centro("NOMBRES",$c2).$arc->centro("DESCUENTO",$c3).$arc->centro("TOTAL",$c4).
		   $arc->enter().$arc->enter();
 $pagina=1;
 ...
 $lista = NULL; $index=0;
 for($i=1;$i<$filas;$i++){		 
  $nombre = trim($Primavacacionesnominaliquidaciones->prestaciones[$i][3]).$arc->espacio(1).trim($Primavacacionesnominaliquidaciones->prestaciones[$i][4]).$arc->espacio(1).trim($Primavacacionesnominaliquidaciones->prestaciones[$i][5]).$arc->espacio(1).trim($Primavacacionesnominaliquidaciones->prestaciones[$i][6]);				 
  $lista[$index]=$arc->centro(number_format($Primavacacionesnominaliquidaciones->prestaciones[$i][2]),$c1).$arc->centro($nombre,$c2).$arc->centro(trim($Primavacacionesnominaliquidaciones->prestaciones[$i][7]),$c3).$arc->derecha(number_format($Primavacacionesnominaliquidaciones->prestaciones[$i][8]),$c4);
  
  $total4=$total4+$Primavacacionesnominaliquidaciones->prestaciones[$i][8]; // total
  $index++;  
 }
