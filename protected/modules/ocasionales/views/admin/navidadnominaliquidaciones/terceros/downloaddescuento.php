<?php
$filas = count($Navidadnominaliquidaciones->prestaciones);
$columnas = count($Navidadnominaliquidaciones->prestaciones[1]);
 ...
 $lista = NULL; $index=0;
 for($i=1;$i<$filas;$i++){		 
  $nombre = trim($Navidadnominaliquidaciones->prestaciones[$i][3]).$arc->espacio(1).trim($Navidadnominaliquidaciones->prestaciones[$i][4]).$arc->espacio(1).trim($Navidadnominaliquidaciones->prestaciones[$i][5]).$arc->espacio(1).trim($Navidadnominaliquidaciones->prestaciones[$i][6]);				 
  $lista[$index]=$arc->centro(number_format($Navidadnominaliquidaciones->prestaciones[$i][2]),$c1).$arc->centro($nombre,$c2).$arc->centro(trim($Navidadnominaliquidaciones->prestaciones[$i][7]),$c3).$arc->derecha(number_format($Navidadnominaliquidaciones->prestaciones[$i][8]),$c4);
  
  $total4=$total4+$Navidadnominaliquidaciones->prestaciones[$i][8]; // total
  $index++;  
 }
