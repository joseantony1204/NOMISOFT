...
foreach($unidades as $unidad){
 ...
 for($i=1;$i<$filas-1;$i++){ 
  ...
  if($neto!=0){
	...
	$basico=NULL;
	$basico[0]=$arc->espacio($c1)."³".$arc->izquierda("Comprobante No. ".$Primavacacionesnominaliquidaciones->liquidacion[$i][13].'-'.$Primavacacionesnominaliquidaciones->liquidacion[$i][1],$c2);
	$basico[1]=$arc->espacio($c1)."³".$arc->izquierda(trim($Primavacacionesnomina->PVNO_PERIODO), $c2);
	...
	$pago=NULL;
	$pago[0]=$arc->izquierda("SUELDO (".number_format($Primavacacionesnominaliquidaciones->liquidacion[$i][4]).")",$c3)."³".$arc->centro($Primavacacionesnominaliquidaciones->liquidacion[$i][2],$c4)."³".$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$i][5]),$c5)."³".$arc->espacio($c6)."³".$arc->espacio($c7);
	$lineapago=1;
		...
		for ($ln=6;$ln<13;$ln++){
		  if ($Primavacacionesnominaliquidaciones->liquidacion[$i][$ln]!=0){
		   ...
		   $pago[$lineapago]=$arc->izquierda($Primavacacionesnominaliquidaciones->liquidacion[0][$ln],$c3)."³".$arc->centro($antig,$c4)."³".$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$i][$ln]),$c5)."³".$arc->espacio($c6)."³".$arc->espacio($c7);
		   $lineapago++;
		  }
		}
		...
		for ($ln=1;$ln<2;$ln++){
			if ($Primavacacionesnominaliquidaciones->parafiscales[$i][$ln]!=0){
				$pago[$lineapago]=$arc->izquierda($Primavacacionesnominaliquidaciones->parafiscales[0][$ln],$c3)."³".$arc->espacio($c4)."³".$arc->espacio($c5)."³".$arc->derecha(number_format($Primavacacionesnominaliquidaciones->parafiscales[$i][$ln]),$c6)."³".$arc->espacio($c7);
				$lineapago++;
			}
		}
		...
		$pago[$lineapago].=$arc->derecha(number_format($Primavacacionesnominaliquidaciones->liquidacion[$i][$columnas-1]),$c5)."³";
		$pago[$lineapago].=$arc->derecha(number_format(($Primavacacionesnominaliquidaciones->parafiscales[$i][$parafisales-1])+($tblD[$i][$columnasTblD-1])),$c6)."³";
