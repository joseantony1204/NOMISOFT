...
class TbPager extends CLinkPager
{
	...
	protected function createPageButtons()
	{
		...
		if ($this->displayFirstAndLast)
			$buttons[] = $this->createPageButton($this->firstPageLabel, 0, 'Primero', $currentPage <= 0, false);

		// prev page
		if (($page = $currentPage - 1) < 0)
			$page = 0;

		$buttons[] = $this->createPageButton($this->prevPageLabel, $page, 'Anterior', $currentPage <= 0, false);
		...
		$buttons[] = $this->createPageButton($this->nextPageLabel, $page, 'Siguiente', $currentPage >= ($pageCount - 1), false);

		// last page
		if ($this->displayFirstAndLast)
			$buttons[] = $this->createPageButton($this->lastPageLabel, $pageCount - 1, 'Ultimo', $currentPage >= ($pageCount - 1), false);
